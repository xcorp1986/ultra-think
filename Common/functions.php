    ...
    if (is_string($name)) {
        ...
        if (is_null($value)) {
            ...
            return str_replace(
                $replace,
                $value,
                isset($_lang[$name]) ? $_lang[$name] : $name
            );
        }
...
function T($template = '', $layer = '')
{
    ...
    if ('' == $file) {
        ...
        $file = substr_count($file, '/') > 1 ? substr_replace(
            $file,
            $depr,
            strrpos($file, '/'),
            1
            ...
            if (is_array($filters)) {
                foreach ($filters as $filter) {
                    if (function_exists($filter)) {
                        $data = is_array($data) ? array_map_recursive(
                            $filter,
                            $data
                        ) : $filter($data); // 参数过滤
                    } else {
                        $data = filter_var(
                            $data,
                            is_int($filter) ? $filter : filter_id($filter)
                        );
                        if (false === $data) {
                            return isset($default) ? $default : null;
                        }
...
function M($name = '', $tablePrefix = '', $connection = '')
{
    ...
    $guid = (is_array($connection) ? implode('', $connection) : $connection)
        .$tablePrefix.$name.'_'.$class;
...
function A($name, $layer = '', $level = 0)
{
    ...
    $level = $level
        ?: ($layer == C('DEFAULT_C_LAYER') ? C('CONTROLLER_LEVEL') : 1);
...
function dump($var, $echo = true, $label = null, $strict = true)
{
    ...
    if (!$strict) {
        if (ini_get('html_errors')) {
            ...
            $output = '<pre>'.$label.htmlspecialchars($output, ENT_QUOTES)
                .'</pre>';
        } else {
...
function U($url = '', $vars = '', $suffix = true, $domain = false)
{
    ...
    if ($url) {
        if (0 === strpos($url, '/')) {// 定义路由
            ...
            $var[$varController] = !empty($path) ? array_pop($path)
                : CONTROLLER_NAME;
            if ($maps = C('URL_ACTION_MAP')) {
                if (isset($maps[strtolower($var[$varController])])) {
                    $maps = $maps[strtolower($var[$varController])];
                    if ($action = array_search(
                        strtolower($var[$varAction]),
                        $maps
                    )) {
            ...
            if ($maps = C('URL_CONTROLLER_MAP')) {
                if ($controller = array_search(
                    strtolower($var[$varController]),
                    $maps
                )) {
            ...
            if ($maps = C('URL_MODULE_MAP')) {
                if ($_module = array_search(
                    strtolower($var[$varModule]),
                    $maps
                )) {
    ...
    if (C('URL_MODEL') == 0) { // 普通模式URL转换
        $url = __APP__.'?'.C('VAR_MODULE')."={$module}&".http_build_query(
                array_reverse($var)
            );
        if ($urlCase) {
            $url = strtolower($url);
...
function data_to_xml($data, $item = 'item', $id = 'id')
{
    ...
    foreach ($data as $key => $val) {
        ...
        $xml .= "<{$key}{$attr}>";
        $xml .= (is_array($val) || is_object($val)) ? data_to_xml(
            $val,
            $item,
            $id
        ) : $val;
...
function session($name = '', $value = '')
{
    ...
    if (is_array($name)) { // session初始化 在session_start 之前调用
        ...
        if (C('SESSION_TYPE')) { // 读取session驱动
            $type = C('SESSION_TYPE');
            $class = strpos($type, '\\') ? $type
                : 'Think\\Session\\Driver\\'.ucwords(strtolower($type));
            $hander = new $class();
        ...
        if ('' === $name) {
            ...
            if (strpos($name, '.')) { // 支持数组
                ...
                return $prefix ? isset($_SESSION[$prefix][$name1][$name2])
                    : isset($_SESSION[$name1][$name2]);
            } else {
                return $prefix ? isset($_SESSION[$prefix][$name])
                    : isset($_SESSION[$name]);
            }
