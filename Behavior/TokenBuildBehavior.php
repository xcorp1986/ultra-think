...
class TokenBuildBehavior
{
    ...
    public function run(&$content)
    {
        if (C('TOKEN_ON')) {
            ...
            $input_token = '<input type="hidden" name="'.$tokenName.'" value="'
                .$tokenKey.'_'.$tokenValue.'" />';
            $meta_token = '<meta name="'.$tokenName.'" content="'.$tokenKey.'_'
                .$tokenValue.'" />';
    ...
    private function getToken()
    {
        ...
        if (isset($_SESSION[$tokenName][$tokenKey])) {// 相同页面不重复生成session
            ...
            $tokenValue = is_callable($tokenType) ? $tokenType(microtime(true))
                : md5(microtime(true));
