    ...
    private static function requireHtmlCache()
    {
        ...
        if (!empty($htmls)) {
            ...
            if (!empty($html)) {
                ...
                define(
                    'HTML_FILE_NAME',
                    HTML_PATH.$rule.C('HTML_FILE_SUFFIX', null, '.html')
                );
    ...
    public static function checkHTMLCache($cacheFile = '', $cacheTime = '')
    {
        if (!is_file($cacheFile) && 'sae' != APP_MODE) {
            return false;
                ...
                (new \Think\View)->parseTemplate()
            ) > Storage::get($cacheFile, 'mtime', 'html')) {
            // 模板文件如果更新静态文件需要更新
            return false;
        } elseif (!is_numeric($cacheTime) && function_exists($cacheTime)) {
            return $cacheTime($cacheFile);
        } elseif ($cacheTime != 0
            && NOW_TIME > Storage::get(
                $cacheFile,
                'mtime',
                'html'
            ) + $cacheTime) {
