...
class ParseTemplateBehavior extends Behavior
{
    ...
    public function run(&$_data)
    {
        ...
        $_content = empty($_data['content']) ? $_data['file']
            : $_data['content'];
        ...
        if ('think' == $engine) { // 采用Think模板引擎
            if ((!empty($_data['content'])
                ...
                //载入模版缓存文件
                Storage::load(
                    C('CACHE_PATH').$_data['prefix'].md5($_content).C(
    ...
    protected function checkCache($tmplTemplateFile, $prefix = '')
    {
        ...
        $tmplCacheFile = C('CACHE_PATH').$prefix.md5($tmplTemplateFile).C(
                'TMPL_CACHFILE_SUFFIX'
            );
        if (!Storage::has($tmplCacheFile)) {
            return false;
        } elseif (filemtime($tmplTemplateFile) > Storage::get(
                $tmplCacheFile,
                'mtime'
            )) {
            // 模板文件如果有更新则缓存需要更新
            return false;
        } elseif (C('TMPL_CACHE_TIME') != 0
            && time() > Storage::get(
                $tmplCacheFile,
                'mtime'
            ) + C('TMPL_CACHE_TIME')) {
        ...
        if (C('LAYOUT_ON')) {
            ...
            if (filemtime($layoutFile) > Storage::get(
                    $tmplCacheFile,
                    'mtime'
                )) {
                return false;
