...
class Model
{
    ...
    public function __construct($name = '', $tablePrefix = '', $connection = '')
    {
        ...
        $this->db(
            0,
            empty($this->connection) ? $connection : $this->connection,
            true
        );
    }
    ...
    public function flush()
    {
        ...
        foreach ($fields as $key => $val) {
            ...
            if ($val['primary']) {
                // 增加复合主键支持
                if (isset($this->fields['_pk'])
                    && $this->fields['_pk'] != null) {
        ...
        if (C('DB_FIELDS_CACHE')) {
            ...
            $db = $this->dbName ?: C('DB_NAME');
            F(
                '_fields/'.strtolower($db.'.'.$this->tablePrefix.$this->name),
                $this->fields
            );
        }
    }
    ...
    public function __call($method, $args)
    {
        if (in_array(strtolower($method), $this->methods, true)) {
            ...
            return $this->getField(
                strtoupper($method).'('.$field.') AS tp_'.$method
            );
            ...
            if (!empty($resultSet)) {
                ...
                foreach ($resultSet as $result) {
                    ...
                    if (2 == $count) {
                        ...
                        $cols[$name] = is_string($sepa) ? implode(
                            $sepa,
    ...
    protected function _parseOptions($options = [])
    {
        ...
        if (isset($options['where']) && is_array($options['where'])
            ...
            foreach ($options['where'] as $key => $val) {
                ...
                } elseif (!is_numeric($key) && '_' != substr($key, 0, 1)
                    && false === strpos($key, '.')
                    && false === strpos($key, '(')
                    && false === strpos($key, '|')
                    && false === strpos($key, '&')) {
    ...
    {
        if (!isset($this->options['bind'][':'.$key])
            && isset($this->fields['_type'][$key])) {
            $fieldType = strtolower($this->fields['_type'][$key]);
            if (false !== strpos($fieldType, 'enum')) {
                // 支持ENUM类型优先检测
            } elseif (false === strpos($fieldType, 'bigint')
     ...
     * @return array
     */
    protected function _read_data(array $data)
    {
        // 检查字段映射
    ...
    public function where($where, $parse = null)
    {
        ...
        if (isset($this->options['where'])) {
            $this->options['where'] = array_merge(
                $this->options['where'],
                $where
            );
        } else {
    ...
    public function create($data = '', $type = '')
    {
        ...
        // 状态
        $type = $type
            ?: (!empty($data[$this->getPk()]) ? self::MODEL_UPDATE
                : self::MODEL_INSERT);
    ...
    protected function autoValidation($data, $type)
    {
        ...
        if (isset($_validate)) {
            ...
            foreach ($_validate as $key => $val) {
                ...
                // 判断是否需要执行验证
                if (empty($val[5]) || ($val[5] == self::MODEL_BOTH && $type < 3)
                    || $val[5] == $type) {
                    ...
                    switch ($val[3]) {
                        ...
                        case self::MUST_VALIDATE:
                            if (false === $this->_validationField(
                                    $data,
                                    $val
                                )) {
                                return false;
    ...
    public function check($value, $rule, $type = 'regex')
    {
        ...
        switch ($type) {
            ...
            case 'notbetween':
                ...
                return $type == 'between' ? $value >= $min && $value <= $max
                    : $value < $min || $value > $max;
            // 验证是否等于某个值
            // 验证是否等于某个值
            case 'equal':
    ...
    public function autoCheckToken($data)
    {
        ...
        if (C('TOKEN_ON')) {
            ...
            if (isset($_SESSION[$name][$key]) && $value
                // 防止重复提交
    ...
    public function join($join, $type = 'INNER')
    {
        ...
        if (is_array($join)) {
            foreach ($join as $key => &$_join) {
                ...
                $_join = false !== stripos($_join, 'JOIN') ? $_join
                    : $type.' JOIN '.$_join;
            }
            ...
            $this->options['join'][] = false !== stripos($join, 'JOIN') ? $join
                : $type.' JOIN '.$join;
        }
    ...
    public function union($union, $all = false)
    {
        ...
        if (is_string($union)) {
            ...
            if (isset($union[0])) {
                $this->options['union'] = array_merge(
                    $this->options['union'],
                    $union
                );
