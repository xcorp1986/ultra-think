...
class Mysql extends Driver
{
    ...
    public function getFields($tableName)
    {
        ...
        if ($result) {
            foreach ($result as $key => $val) {
                if (\PDO::CASE_LOWER != $this->_linkID->getAttribute(
                        \PDO::ATTR_CASE
                    )) {
    ...
    public function insertAll($dataSet, $options = [], $replace = false)
    {
        ...
        $sql = (true === $replace ? 'REPLACE' : 'INSERT').' INTO '
            ...
            .') VALUES '.implode(',', $values).$this->parseDuplicate($replace);
    ...
    protected function parseDuplicate($duplicate)
    {
        ...
        foreach ((array)$duplicate as $key => $val) {
            if (is_numeric(
                ...
                $updates[] = $this->parseKey($val)."=VALUES(".$this->parseKey(
                        $val
                    ).")";
            } else {
