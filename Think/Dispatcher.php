...
final class Dispatcher
{
    ...
    public static function dispatch()
    {
        ...
        if (empty($_SERVER['PATH_INFO'])) {
            ...
            define(
                '__EXT__',
                strtolower(pathinfo($_SERVER['PATH_INFO'], PATHINFO_EXTENSION))
            );
            ...
            if (!defined('BIND_MODULE')
                ...
                if (__INFO__ && C('MULTI_MODULE')) {
                    ...
                    if (empty($allowList)
                        || (is_array($allowList)
                            && in_array_case($module, $allowList))) {
        ...
        define(
            'MODULE_NAME',
            defined('BIND_MODULE') ? BIND_MODULE : self::getModule($varModule)
        );
        ...
        if (MODULE_NAME
            ...
            if ('common' != APP_MODE
                && is_file(
                    MODULE_PATH.'Conf/config_'.APP_MODE.CONF_EXT
                )) {
            ...
            if (APP_STATUS
                && is_file(
                    MODULE_PATH.'Conf/'.APP_STATUS.CONF_EXT
                )) {
        ...
        define(
            '__MODULE__',
            (defined('BIND_MODULE') || !C('MULTI_MODULE'))
                ? __APP__
                : __APP__.'/'.($urlCase ? strtolower($moduleName)
                    : $moduleName)
        );
        ...
        if ('' != $_SERVER['PATH_INFO']
            ...
            $_SERVER['PATH_INFO'] = preg_replace(
                C('URL_HTML_SUFFIX') ? '/\.('.trim(C('URL_HTML_SUFFIX'), '.')
                    .')$/i' : '/\.'.__EXT__.'$/i',
                '',
                $_SERVER['PATH_INFO']
            );
        ...
        // 当前控制器的UR地址
        $controllerName = defined('CONTROLLER_ALIAS') ? CONTROLLER_ALIAS
            : CONTROLLER_NAME;
        define(
            '__CONTROLLER__',
            __MODULE__.$depr.(defined('BIND_CONTROLLER')
                ? ''
                : ($urlCase ? parse_name($controllerName)
                    : $controllerName))
        );
        ...
        define(
            '__ACTION__',
            __CONTROLLER__.$depr.(defined('ACTION_ALIAS') ? ACTION_ALIAS
                : ACTION_NAME)
        );
