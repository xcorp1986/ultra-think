...
class Cx extends TagLib
{
    ...
    public function _volist($tag, $content)
    {
        ...
        $parseStr .= '<?php endforeach; endif; else: echo "'.$empty
            .'" ;endif; ?>';
    ...
    public function _foreach($tag, $content)
    {
        ...
        $name = $this->autoBuildVar($name);
        $parseStr = '<?php if(is_array('.$name.')): foreach('.$name.' as $'.$key
            .'=>$'.$item.'): ?>';
        $parseStr .= $this->tpl->parse($content);
     ...
     * 格式： <range name="var|function"  value="val" type='in|notin'
     * >content</range> example: <range name="a"  value="1,2,3" type='in'
     * >content</range>
     *
     *
     * @param array  $tag     标签属性
    ...
    public function _range($tag, $content, $type = 'in')
    {
        ...
        if ($type == 'between') {
            $parseStr = '<?php $_RANGE_VAR_='.$str.';if('.$name
                .'>= $_RANGE_VAR_[0] && '.$name.'<= $_RANGE_VAR_[1]):?>'
                .$content.'<?php endif; ?>';
        } elseif ($type == 'notbetween') {
            $parseStr = '<?php $_RANGE_VAR_='.$str.';if('.$name
                .'<$_RANGE_VAR_[0] || '.$name.'>$_RANGE_VAR_[1]):?>'.$content
                .'<?php endif; ?>';
        } else {
    ...
    public function _notempty($tag, $content)
    {
        $name = $tag['name'];
        $name = $this->autoBuildVar($name);
        $parseStr = '<?php if(!empty('.$name.')): ?>'.$content
            .'<?php endif; ?>';
    ...
    public function _import($tag, $content, $isFile = false, $type = '')
    {
        ...
        if ($isFile) {
            ...
            foreach ($array as $val) {
                ...
                switch ($type) {
                    case 'js':
                        $parseStr .= '<script src="'.$val
                            .'"></script>';
                        break;
                    case 'css':
                        $parseStr .= '<link rel="stylesheet" type="text/css" href="'
                            .$val.'" />';
                        break;
                    case 'php':
    ...
    public function _for($tag, $content)
    {
        ...
        $parseStr = '<?php $__FOR_START_'.$rand.'__='.$start.';$__FOR_END_'
            .$rand.'__='.$end.';';
